...
function bookacti_get_booking_system_data( $atts ) {
	...
	if( $atts[ 'auto_load' ] && ! $atts[ 'custom_dataset' ] ) {
		...
		// Check if the display period starts before it ends
		$is_valid_display_period = true;
		...
		if( ! $booking_system_data[ 'no_events' ] ) {
			...
			if( $atts[ 'days_off' ] ) {
				$events_from    = isset( $display_period[ 'start_first' ] ) ? $display_period[ 'start_first' ] : ( isset( $display_period[ 'start' ] ) ? $display_period[ 'start' ] : $events_interval[ 'start' ] );
				$events_to      = isset( $display_period[ 'end_last' ] ) ? $display_period[ 'end_last' ] : ( isset( $display_period[ 'end' ] ) ? $display_period[ 'end' ] : $events_interval[ 'end' ] );
				$events_from_dt = new DateTime( $events_from, $timezone );
				$events_to_dt   = new DateTime( $events_to, $timezone );
				...
				foreach( $atts[ 'days_off' ] as $off_period ) {
					...
					foreach( $events[ 'events' ] as $i => $event ) {
						...
						$event_end_dt   = new DateTime( $event[ 'end' ], $timezone );
			...
			if( $booking_system_data[ 'trim' ] && $events[ 'events' ] ) {
				...
				if( $is_last_interval && $last_event_dt && $last_event_dt < $end_dt ) {
					$booking_system_data[ 'end' ] = $last_event_dt->format( 'Y-m-d H:i:s' );
				}
			}
...
function bookacti_get_calendar_html( $booking_system_data = array() ) {
	$hide_calendar = in_array( 'preselected_event', $booking_system_data[ 'hide_calendar' ], true ) && $booking_system_data[ 'picked_events' ];
	
	ob_start();
	?>
		<div class='bookacti-calendar-title bookacti-booking-system-title <?php if( $hide_calendar ) { echo 'bookacti-hidden-field'; } ?>'>
			<?php echo bookacti_get_message( 'calendar_title' ); ?>
		</div>
		<div class='bookacti-calendar <?php if( $hide_calendar ) { echo 'bookacti-hidden-field'; } ?>'></div>
	<?php
...
function bookacti_format_booking_system_attributes( $raw_atts = array() ) {
	...
	// Sanitize booleans
	$booleans_to_check = array( 'multiple_bookings', 'bookings_only', 'tooltip_booking_list', 'groups_only', 'groups_single_events', 'groups_first_event_only', 'auto_load', 'trim', 'out_of_period_events', 'past_events', 'past_events_bookable', 'select_first_event', 'check_roles' );
	...
	// Format the hide calendar value
	$available_hide_calendar_options   = array_keys( bookacti_get_available_hide_calendar_options() );
	$formatted_atts[ 'hide_calendar' ] = array_values( array_intersect( $available_hide_calendar_options, bookacti_str_ids_to_array( $atts[ 'hide_calendar' ] ) ) );
	if( ! $formatted_atts[ 'hide_calendar' ] || array_intersect( $formatted_atts[ 'hide_calendar' ], array( false, 'none', 'false', 'no' ) ) ) { $formatted_atts[ 'hide_calendar' ] = array( 'none' ); }
...
function bookacti_get_booking_system_fields_default_data( $fields = array() ) {
	...
	// Form Action
	if( ! $fields || in_array( 'form_action', $fields, true ) ) {
		$defaults[ 'form_action' ] = array(
			'type'    => 'select',
			'name'    => 'form_action',
			...
			'title'   => esc_html__( 'Form action', 'booking-activities' ),
			'tip'     => esc_html__( 'What action should this form perform?', 'booking-activities' )
		);
	}
	
	// When to perform form action
	if( ! $fields || in_array( 'when_perform_form_action', $fields, true ) ) {
		$defaults[ 'when_perform_form_action' ] = array(
			'type'    => 'select',
			...
			'title'   => esc_html__( 'When to perform the action', 'booking-activities' ),
	...
	if( ! $fields || in_array( 'select_first_event', $fields, true ) ) {
		$defaults[ 'select_first_event' ] = array(
			'type'  => 'checkbox',
			'name'  => 'select_first_event',
			'id'    => 'bookacti-select_first_event',
			'title' => esc_html__( 'Auto select first event', 'booking-activities' ),
			'tip'   => esc_html__( 'Automatically select the first available event displayed.', 'booking-activities' )
		);
	}
	...
	// Hide calendar
	if( ! $fields || in_array( 'hide_calendar', $fields, true ) ) {
		$defaults[ 'hide_calendar' ] = array(
			'type'    => 'select',
			'name'    => 'hide_calendar',
			'id'      => 'bookacti-hide_calendar',
			'options' => array( 
				'none'              => esc_html__( 'Never', 'booking-activities' ), 
				'preselected_event' => esc_html__( 'If an event is automatically selected', 'booking-activities' )
			),
			'title'   => esc_html__( 'Hide the calendar', 'booking-activities' ),
			'tip'     => esc_html__( 'Hide the calendar if an event has already been selected automatically.', 'booking-activities' )
		);
	}
 ...
 * @return array
 */
function bookacti_get_fullcalendar_fields_default_data( $fields = array() ) {
	if( ! is_array( $fields ) ) { $fields = array(); }
	$defaults = array();
	...
	return apply_filters( 'bookacti_fullcalendar_fields_default_data', $defaults, $fields );
}
 ...
 *  @type int $form_id Set your form id to validate the event against its form parameters. Default is 0: ignore form validation.
 * }
 * @return array
 */
function bookacti_validate_picked_events( $picked_events, $args = array() ) {
	$validated = array( 
		'status'   => 'failed', 
		'error'    => 'invalid_event',
		'messages' => array()
	...
	if( $args[ 'form_id' ] && count( $picked_events ) > 1 ) {
		$calendar_data = bookacti_get_form_field_data_by_name( $args[ 'form_id' ], 'calendar' );
		$allow_multiple_bookings = ! empty( $calendar_data[ 'multiple_bookings' ] );
	}
	$allow_multiple_bookings = apply_filters( 'bookacti_allow_multiple_bookings', $allow_multiple_bookings, $picked_events, $args );
		...
		foreach( $picked_events as $i => $picked_event ) {
			...
			if( ! $exists ) {
				if( $group_id ) {
					...
					$validated[ 'messages' ][ 'do_not_exist' ] = array( sprintf( esc_html__( 'The group of events "%s" doesn\'t exist, please pick an event and try again.', 'booking-activities' ), $title ? $title . ' (' . $dates . ')' : $dates ) );
				} else {
					/* translators: %s = The event title and dates. E.g.: The event "Basketball (Sep, 22nd - 3:00 PM to 6:00 PM)" doesn't exist. */
					$validated[ 'messages' ][ 'do_not_exist' ] = array( sprintf( esc_html__( 'The event "%s" doesn\'t exist, please pick an event and try again.', 'booking-activities' ), $title ? $title . ' (' . $dates . ')' : $dates ) );
				}
				continue;
			}
			...
			if( $args[ 'form_id' ] ) { 
				...
				if( $group_id ) {
					$form_validated = bookacti_is_picked_group_of_events_available_on_form( $picked_event, $group, $args[ 'form_id' ] );
				} else if( is_numeric( $group_id ) ) {
					$form_validated = bookacti_is_picked_event_available_on_form( $picked_event[ 'events' ][ 0 ], $event, $args[ 'form_id' ] );
				}
				...
				if( $form_validated[ 'status' ] !== 'success' ) {
					$validated[ 'messages' ] = array_merge( $validated[ 'messages' ], $form_validated[ 'messages' ] );
					continue;
				}
			}
			...
			if( $args[ 'quantity' ] <= $availability )                                                       { $is_qty_inf_to_avail = true; }
			if( $min_quantity === 0 || ( $args[ 'quantity' ] + $quantity_already_booked ) >= $min_quantity ) { $is_qty_sup_to_min = true; }
			if( $max_quantity === 0 || $args[ 'quantity' ] <= ( $max_quantity - $quantity_already_booked ) ) { $is_qty_inf_to_max = true; }
			...
			$validated = apply_filters( 'bookacti_validate_picked_event', $validated, $picked_events[ $i ], $args );
		}
	}
	...
	return apply_filters( 'bookacti_validate_picked_events', $validated, $picked_events, $args );
}
		...
		if( count( $occurrences[ $group_date ] ) === count( $picked_event_group[ 'events' ] ) ) {
			...
			foreach( $occurrences[ $group_date ] as $group_event ) {
				...
				foreach( $picked_event_group[ 'events' ] as $picked_event ) {
					if( intval( $picked_event[ 'id' ] ) === intval( $group_event[ 'id' ] )
...
function bookacti_is_picked_event_available_on_form( $picked_event, $event_data, $form_id ) {
	$validated = array( 'status' => 'failed', 'messages' => array() );
	
	// Check if the event exists
	if( ! $event_data ) {
		$validated[ 'messages' ][ 'unknown_event' ] = array( esc_html__( 'Invalid event ID.', 'booking-activities' ) );
	}
	...
	if( ! $calendar_data ) {
		$validated[ 'messages' ][ 'invalid_form' ] = array( esc_html__( 'Failed to retrieve the requested form data.', 'booking-activities' ) );
	}
	
	// If the form calendar doesn't have the event template or the event activity
	$template_ids = ! empty( $calendar_data[ 'calendars' ] ) ? bookacti_ids_to_array( $calendar_data[ 'calendars' ] ) : array();
	$activity_ids = ! empty( $calendar_data[ 'activities' ] ) ? bookacti_ids_to_array( $calendar_data[ 'activities' ] ) : array();
	$template_id  = ! empty( $event_data[ 'template_id' ] ) ? intval( $event_data[ 'template_id' ] ) : 0;
	$activity_id  = ! empty( $event_data[ 'activity_id' ] ) ? intval( $event_data[ 'activity_id' ] ) : 0;
	
	if( ( $template_ids && ! in_array( $template_id, $template_ids, true ) )
	||  ( $activity_ids && ! in_array( $activity_id, $activity_ids, true ) ) ) {
		$validated[ 'messages' ][ 'event_not_in_form' ] = array( esc_html__( 'The selected event is not supposed to be available on this form.', 'booking-activities' ) );
	}
	
	// If the form calendar have groups, with no possibility to book a single event
	$category_ids         = isset( $calendar_data[ 'group_categories' ] ) && is_array( $calendar_data[ 'group_categories' ] ) && ! in_array( 'none', $calendar_data[ 'group_categories' ], true ) ? bookacti_ids_to_array( $calendar_data[ 'group_categories' ] ) : array( 'none' );
	...
	if( ! in_array( 'none', $category_ids, true ) && ! $groups_single_events ) {
		if( $groups_only ) {
			...
			$groups_interval = bookacti_sanitize_events_interval( array( 'start' => $picked_event[ 'start' ], 'end' => $picked_event[ 'end' ] ) );
			$groups          = bookacti_get_groups_of_events( array( 'templates' => array( $template_id ), 'group_categories' => $category_ids, 'interval' => $groups_interval, 'interval_started' => 1, 'past_events' => 1 ) );
			...
			if( $is_in_group ) {
				$validated[ 'messages' ][ 'single_event_not_allowed' ] = array( esc_html__( 'The selected event is part of a group and cannot be booked alone.', 'booking-activities' ) );
			}
		}
	}
	
	$timezone                  = new DateTimeZone( bookacti_get_setting_value( 'bookacti_general_settings', 'timezone' ) );
	$event_start_dt            = new DateTime( $picked_event[ 'start' ], $timezone );
	...
	if( $days_off ) {
		...
		foreach( $days_off as $off_period ) {
			...
			if( ( $event_start_dt >= $off_from && $event_start_dt <= $off_to ) 
			||  ( ! $started_days_off_bookable && $event_start_dt < $off_from && $event_end_dt > $off_from ) ) {
				...
				if( ! isset( $validated[ 'messages' ][ 'event_in_days_off' ] ) ) {
					$validated[ 'messages' ][ 'event_in_days_off' ] = array();
				}
				
				if( $off_from_formatted === $off_to_formatted ) {
					/* translators: %s is a formatted date and time (e.g.: "February 15, 2022 11:06 am") */
					$validated[ 'messages' ][ 'event_in_days_off' ][] = sprintf( esc_html__( 'You cannot book an event taking place on %s.', 'booking-activities' ), $off_from_formatted );
				} else {
					/* translators: %1$s and %2$s are formatted dates and times (e.g.: "February 15, 2022 11:06 am") */
					$validated[ 'messages' ][ 'event_in_days_off' ][] = sprintf( esc_html__( 'You cannot book an event taking place between %1$s and %2$s.', 'booking-activities' ), $off_from_formatted, $off_to_formatted );
				}
			}
		}
	}
	...
	$past_events_bookable    = ! empty( $calendar_data[ 'past_events' ] ) && ! empty( $calendar_data[ 'past_events_bookable' ] );
	$started_events_bookable = bookacti_get_setting_value( 'bookacti_general_settings', 'started_events_bookable' );
	...
	return apply_filters( 'bookacti_is_picked_event_available_on_form', $validated, $picked_event, $event_data, $form_id );
}
...
function bookacti_is_picked_group_of_events_available_on_form( $picked_event_group, $group_data, $form_id ) {
	...
	if( ! $group_data ) {
		$validated[ 'messages' ][ 'unknown_group_of_events' ] = array( esc_html__( 'Invalid group of events ID.', 'booking-activities' ) );
	}
	...
	$template_ids = ! empty( $calendar_data[ 'calendars' ] ) ? bookacti_ids_to_array( $calendar_data[ 'calendars' ] ) : array();
	$category_ids = isset( $calendar_data[ 'group_categories' ] ) && is_array( $calendar_data[ 'group_categories' ] ) && ! in_array( 'none', $calendar_data[ 'group_categories' ], true ) ? bookacti_ids_to_array( $calendar_data[ 'group_categories' ] ) : array( 'none' );
	$template_id  = ! empty( $group_data[ 'template_id' ] ) ? intval( $group_data[ 'template_id' ] ) : 0;
	$category_id  = ! empty( $group_data[ 'category_id' ] ) ? intval( $group_data[ 'category_id' ] ) : 0;
	
	// If the form calendar doesn't have the group of events' template
	if( $template_ids && ! in_array( $template_id, $template_ids, true ) ) {
		$validated[ 'messages' ][ 'events_not_in_form' ] = array( esc_html__( 'The selected events are not supposed to be available on this form.', 'booking-activities' ) );
	}
	
	// If the form calendar doesn't have groups
	if( in_array( 'none', $category_ids, true ) ) {
		$validated[ 'messages' ][ 'groups_not_allowed' ] = array( esc_html__( 'You cannot book groups of events with this form, you must select a single event.', 'booking-activities' ) );
	}
	...
	// If the form calendar have groups
		...
		if( ! $category_ids ) {
			$category_ids = bookacti_ids_to_array( bookacti_get_group_category_ids_by_template( array( $group_data[ 'template_id' ] ) ) );
		}

		// Check if the group of event category is available on this form
		if( ! in_array( $category_id, $category_ids, true ) ) {
			$validated[ 'messages' ][ 'group_not_in_form' ] = array( esc_html__( 'The selected goup of events is not supposed to be available on this form.', 'booking-activities' ) );
		}
	}
	...
	if( $days_off ) {
		...
		foreach( $picked_event_group[ 'events' ] as $grouped_event ) {
			...
			foreach( $days_off as $off_period ) {
				$off_from = new DateTime( $off_period[ 'from' ] . ' 00:00:00', $timezone );
				$off_to   = new DateTime( $off_period[ 'to' ] . ' 23:59:59', $timezone );
				if( ( $grouped_event_start_dt >= $off_from && $grouped_event_start_dt <= $off_to )
				||  ( ! $started_days_off_bookable && $grouped_event_start_dt < $off_from && $grouped_event_end_dt > $off_from ) ) {
					$off_from_formatted = bookacti_format_datetime( $off_from->format( 'Y-m-d H:i:s' ), $date_format );
					$off_to_formatted   = bookacti_format_datetime( $off_to->format( 'Y-m-d H:i:s' ), $date_format );
					...
					if( $off_from_formatted === $off_to_formatted ) {
						/* translators: %s is a formatted date and time (e.g.: "February 15, 2022 11:06 am") */
						$validated[ 'messages' ][ 'event_in_days_off' ][] = sprintf( esc_html__( 'You cannot book a group if any of its events takes place on %s.', 'booking-activities' ), $off_from_formatted );
					} else {
						/* translators: %1$s and %2$s are formatted dates and times (e.g.: "February 15, 2022 11:06 am") */
						$validated[ 'messages' ][ 'event_in_days_off' ][] = sprintf( esc_html__( 'You cannot book a group if any of its events takes place between %1$s and %2$s.', 'booking-activities' ), $off_from_formatted, $off_to_formatted );
					}
				}
			}
		}
	...
	return apply_filters( 'bookacti_is_picked_group_of_events_available_on_form', $validated, $picked_event_group, $group_data, $form_id );
}
...
function bookacti_get_events_array_from_db_events( $events, $raw_args = array() ) {
	...
	if( $args[ 'bounding_only' ] && ! $args[ 'data_only' ] ) {
		$events = bookacti_get_bounding_events_from_db_events( $events, $args );
	}
	if( ! $events ) { return $events_array; }
...
function bookacti_get_bounding_events_from_db_events( $events, $raw_args = array() ) {
	...
	foreach( $events as $event ) {
		...
		if( ! empty( $event->repeat_freq ) && $event->repeat_freq !== 'none' && ! empty( $event->repeat_from ) && ! empty( $event->repeat_to ) ) { 
			...
			$duration       = bookacti_php_date_interval_in_seconds( $event_start_dt->diff( $event_end_dt, true ) );
...
function bookacti_get_occurrences_of_repeated_event( $event, $raw_args = array() ) {
	...
	$timezone          = new DateTimeZone( bookacti_get_setting_value( 'bookacti_general_settings', 'timezone' ) );
	$interval_start    = ! empty( $args[ 'interval' ][ 'start' ] ) ? new DateTime( $args[ 'interval' ][ 'start' ], $timezone ) : false;
	$interval_end      = ! empty( $args[ 'interval' ][ 'end' ] ) ? new DateTime( $args[ 'interval' ][ 'end' ], $timezone ) : false;
	...
	while( $iterate ) {
		...
		// Check if the occurrence is in an exception
		if( $args[ 'skip_exceptions' ] && $repeat_exceptions && bookacti_is_date_in_days_off( $occurrence_start->format( 'Y-m-d' ), $repeat_exceptions ) ) { continue; }
...
function bookacti_get_new_interval_of_events( $availability_period, $min_interval = array(), $interval_duration = 0, $past_events = false ) {
	...
	$calendar_start = ! empty( $availability_period[ 'start' ] ) ? new DateTime( $availability_period[ 'start' ], $timezone ) : false;
	$calendar_end   = ! empty( $availability_period[ 'end' ] ) ? new DateTime( $availability_period[ 'end' ], $timezone ) : false;
	
	if( ! $past_events && $calendar_end && $calendar_end < $current_time ) { return array(); }
	
	if( empty( $min_interval[ 'start' ] ) || empty( $min_interval[ 'end' ] ) ) { $min_interval = array(); } 
	if( ! $min_interval ) {
		if( $calendar_start && $calendar_start > $current_time ) {
			$min_interval = array( 'start' => $availability_period[ 'start' ], 'end' => $availability_period[ 'start' ] );
		} else if( $calendar_end && $calendar_end < $current_time ) {
	...
	if( $past_events ) {
		...
		if( $calendar_start && $calendar_start > $interval_start ) {
			$interval_end_days_to_add += abs( intval( $interval_start->diff( $calendar_start )->format( '%a' ) ) );
		}
	} else {
		$interval_end_days_to_add += $half_interval;
	}
	if( $calendar_start && $calendar_start > $interval_start ) { $interval_start = clone $calendar_start; }
	
	// Compute interval end
	$interval_end->add( new DateInterval( 'P' . $interval_end_days_to_add . 'D' ) );
	if( $calendar_end && $calendar_end < $interval_end ) { $interval_end = clone $calendar_end; }
...
function bookacti_get_calendar_field_availability_period( $calendar_field ) {
	...
	$absolute_period = array(
		'start' => ! empty( $calendar_field[ 'start' ] ) ? bookacti_sanitize_datetime( $calendar_field[ 'start' ] ) : ( empty( $calendar_field[ 'past_events' ] ) || empty( $calendar_field[ 'past_events_bookable' ] ) ? $current_time->format( 'Y-m-d H:i:s' ) : '' ),
...
function bookacti_get_availability_period( $absolute_period = array(), $relative_period = array() ) {
	...
	$calendar_start_date = ! empty( $absolute_period[ 'start' ] ) ? $absolute_period[ 'start' ] : '';
	$calendar_end_date   = ! empty( $absolute_period[ 'end' ] ) ? $absolute_period[ 'end' ] : '';
	...
	if( $relative_period ) { 
		...
		if( $relative_period_start > 0 ) {
			...
			$calendar_start_dt = $calendar_start_date ? new DateTime( $calendar_start_date, $timezone ) : false;
			if( ! $calendar_start_dt || ( $calendar_start_dt && $relative_start_dt > $calendar_start_dt ) ) {
				$calendar_start_date = $relative_start_dt->format( 'Y-m-d H:i:s' );
			}
		}
		if( $relative_period_end > 0 ) {
			...
			$calendar_end_dt = $calendar_end_date ? new DateTime( $calendar_end_date, $timezone ) : false;
			if( ! $calendar_end_dt || ( $calendar_end_dt && $relative_end_dt < $calendar_end_dt ) ) {
				$calendar_end_date = $relative_end_dt->format( 'Y-m-d H:i:s' );
			}
		}
	}
...
function bookacti_get_booking_system_display_period( $booking_system_data ) {
	...
	if( $display_period[ 'start' ] !== $display_period[ 'end' ] ) {
...
/**
 * Get the first event or group of events available of the booking system (among the loaded ones)
 * @since 1.17.0
 * @param array $booking_system_data
 * @return array
 */
function bookacti_get_booking_system_first_available_picked_events( $booking_system_data ) {
	$picked_events = array();
	
	if( ! $booking_system_data[ 'no_events' ] ) {
		$first_event_dt = null;
		
		if( $booking_system_data[ 'events' ] && ( ! $booking_system_data[ 'groups_only' ] || $booking_system_data[ 'groups_single_events' ] ) ) {
			foreach( $booking_system_data[ 'events' ] as $event ) {
				$is_available = bookacti_is_booking_system_event_available( $booking_system_data, $event );
				if( $is_available ) {
					$first_event_dt = DateTime::createFromFormat( 'Y-m-d H:i:s', $event[ 'start' ] );
					$picked_events  = array( $event );
					break;
				}
			}
		}
		
		if( $booking_system_data[ 'groups_events' ] ) {
			foreach( $booking_system_data[ 'groups_events' ] as $group_id => $group_events_by_date ) {
				foreach( $group_events_by_date as $group_date => $group_events ) {
					// If an event is already picked, check if the group starts after the picked event
					$first_group_event    = reset( $group_events );
					$first_group_event_dt = DateTime::createFromFormat( 'Y-m-d H:i:s', $first_group_event[ 'start' ] );
					if( $picked_events && $first_group_event_dt >= $first_event_dt ) {
						break;
					}
					
					$is_available = bookacti_is_booking_system_group_of_events_available( $booking_system_data, $group_id, $group_date );
					if( $is_available ) {
						$first_event_dt = clone $first_group_event_dt;
						$group_picked_events = array();
						foreach( $group_events as $group_event ) {
							$group_picked_events[] = array_merge( $group_event, array( 'group_id' => $group_id, 'group_date' => $group_date ) );
						}
						$picked_events = $group_picked_events;
						break;
					}
				}
			}
	...
	return apply_filters( 'bookacti_booking_system_first_available_picked_events', $picked_events, $booking_system_data );
}


/**
 * Check if an event is available based on the booking system data only
 * @since 1.17.0
 * @param array $booking_system_data
 * @param array|object $event
 * @return bool
 */
function bookacti_is_booking_system_event_available( $booking_system_data, $event ) {
	$event        = (array) $event;
	$event_id     = ! empty( $event[ 'id' ] ) ? intval( $event[ 'id' ] ) : 0;
	$event_start  = ! empty( $event[ 'start' ] ) ? $event[ 'start' ] : '';
	$event_end    = ! empty( $event[ 'end' ] ) ? $event[ 'end' ] : '';
	$event_data   = ! empty( $booking_system_data[ 'events_data' ][ $event_id ] ) ? $booking_system_data[ 'events_data' ][ $event_id ] : array();
	$total_avail  = ! empty( $event_data[ 'availability' ] ) ? intval( $event_data[ 'availability' ] ) : 0;
	$bookings_nb  = ! empty( $booking_system_data[ 'bookings' ][ $event_id ][ $event_start ][ 'quantity' ] ) ? intval( $booking_system_data[ 'bookings' ][ $event_id ][ $event_start ][ 'quantity' ] ) : 0;
	$availability = $total_avail ? $total_avail - $bookings_nb : 0;
	
	// Check if the event is explicitly set as unavailable
	$is_event_available = true;
	$is_event_found     = false;
	if( $booking_system_data[ 'events' ] ) {
		foreach( $booking_system_data[ 'events' ] as $bs_event ) {
			if( $event_id === intval( $bs_event[ 'id' ] ) && $event_start === $bs_event[ 'start' ] && $event_end === $bs_event[ 'end' ] ) {
				if( isset( $bs_event[ 'is_available' ] ) && ! intval( $bs_event[ 'is_available' ] ) ) {
	...
	$is_available = $event_data && $is_event_found && $is_event_available && $availability > 0;
	...
	if( $is_available ) {
		...
		$is_in_group = false;
		if( $booking_system_data[ 'groups_events' ] ) {
			foreach( $booking_system_data[ 'groups_events' ] as $group_id => $group_occurrences ) {
				foreach( $group_occurrences as $group_date => $group_events ) {
					foreach( $group_events as $group_event ) {
						if( intval( $group_event[ 'id' ] ) === $event_id
						&&  $group_event[ 'start' ] === $event_start
						&&  $group_event[ 'end' ] === $event_end ) {
							$is_in_group = true;
							return false; // Break the loop
						}
						if( $booking_system_data[ 'groups_first_event_only' ] ) {
							return false; // Break the loop
						}
					}
		...
		if( ! $is_in_group || ( $is_in_group && $booking_system_data[ 'groups_single_events' ] ) ) {
			// Get current time
			$timezone = new DateTimeZone( bookacti_get_setting_value( 'bookacti_general_settings', 'timezone' ) );
			$now      = new DateTime( 'now', $timezone );
			
			$started_events_bookable = bookacti_get_setting_value( 'bookacti_general_settings', 'started_events_bookable' );
			$event_start_dt = DateTime::createFromFormat( 'Y-m-d H:i:s', $event_start, $timezone );
			$event_end_dt   = DateTime::createFromFormat( 'Y-m-d H:i:s', $event_end, $timezone );
			
			// Check if the event is past
			$is_past = false;
			$past_events_bookable = $booking_system_data[ 'past_events' ] && $booking_system_data[ 'past_events_bookable' ];
			if( ! $past_events_bookable && $event_start_dt < $now
			&& ! ( $started_events_bookable && $event_end_dt > $now ) ) {
				$is_past = true;
			}
			
			// Get availability period
			$period_asb_start = ! empty( $booking_system_data[ 'availability_period' ][ 'start' ] ) ? DateTime::createFromFormat( 'Y-m-d H:i:s', $booking_system_data[ 'availability_period' ][ 'start' ], $timezone ) : null;
			$period_asb_end   = ! empty( $booking_system_data[ 'availability_period' ][ 'end' ] ) ? DateTime::createFromFormat( 'Y-m-d H:i:s', $booking_system_data[ 'availability_period' ][ 'end' ], $timezone ) : null;
			$period_start     = ! empty( $booking_system_data[ 'start' ] ) ? DateTime::createFromFormat( 'Y-m-d H:i:s', $booking_system_data[ 'start' ], $timezone ) : null;
			$period_end       = ! empty( $booking_system_data[ 'end' ] ) ? DateTime::createFromFormat( 'Y-m-d H:i:s', $booking_system_data[ 'end' ], $timezone ) : null;
			
			// Use the smaller period between the absolute availability period and the calendar start and end dates
			$availability_period = array(
				'start' => ! $period_asb_start && ! $period_start ? '' : ( $period_asb_start && ( ! $period_start || ( $period_start && $period_start < $period_asb_start ) ) ? $period_asb_start : $period_start ),
				'end'   => ! $period_asb_end && ! $period_end ? '' : ( $period_asb_end && ( ! $period_end || ( $period_end && $period_end > $period_asb_end ) ) ? $period_asb_end : $period_end )
			);
			
			// Check if the event is in the availability period
			$is_out_of_period = false;
			if( ( $availability_period[ 'start' ] && $event_start_dt < $availability_period[ 'start' ] && $event_start_dt > $now )
			||  ( $availability_period[ 'end' ] && $event_start_dt > $availability_period[ 'end' ] ) ) {
				$is_out_of_period = true;
			}
			
			// Check days off
			$is_in_days_off = false;
			if( $is_available && $booking_system_data[ 'days_off' ] ) {
				$started_days_off_bookable = bookacti_get_setting_value( 'bookacti_general_settings', 'started_days_off_bookable' );
				foreach( $booking_system_data[ 'days_off' ] as $off_period ) {
					$off_from = new DateTime( $off_period[ 'from' ] . ' 00:00:00', $timezone );
					$off_to   = new DateTime( $off_period[ 'to' ] . ' 23:59:59', $timezone );
			...
			if( ! $is_past && ! $is_out_of_period && ! $is_in_days_off ) {
				$activity_id   = intval( $booking_system_data[ 'events_data' ][ $event_id ][ 'activity_id' ] );
				$activity_data = $booking_system_data[ 'activities_data' ][ $activity_id ][ 'settings' ];
				
				// Check the min and max quantity allowed AND
				// Check the max number of different users allowed
				$min_qty_ok = $max_qty_ok = $max_users_ok = true;
				if( ! empty( $booking_system_data[ 'bookings' ][ $event_id ][ $event_start ] ) ) {
					$min_quantity = ! empty( $activity_data[ 'min_bookings_per_user' ] ) ? intval( $activity_data[ 'min_bookings_per_user' ] ) : 0;
					$max_quantity = ! empty( $activity_data[ 'max_bookings_per_user' ] ) ? intval( $activity_data[ 'max_bookings_per_user' ] ) : 0;
					$max_users    = ! empty( $activity_data[ 'max_users_per_event' ] ) ? intval( $activity_data[ 'max_users_per_event' ] ) : 0;
					
					if( $min_quantity || $max_quantity || $max_users ) {
						$occurrence = $booking_system_data[ 'bookings' ][ $event_id ][ $event_start ];
						$qty_booked = intval( $occurrence[ 'current_user_bookings' ] );
...
/**
 * Check if a group of events is available based on the booking system data only
 * @since 1.17.0
 * @param array $booking_system_data
 * @param array|object $event
 * @return bool
 */
function bookacti_is_booking_system_group_of_events_available( $booking_system_data, $group_id, $group_date ) {
	// Get group data
	$group_data            = ! empty( $booking_system_data[ 'groups_data' ][ $group_id ] ) ? $booking_system_data[ 'groups_data' ][ $group_id ] : array();
	$group_events          = ! empty( $booking_system_data[ 'groups_events' ][ $group_id ][ $group_date ] ) ? $booking_system_data[ 'groups_events' ][ $group_id ][ $group_date ] : array();
	$group_bookings_data   = ! empty( $booking_system_data[ 'groups_bookings' ][ $group_id ][ $group_date ] ) ? $booking_system_data[ 'groups_bookings' ][ $group_id ][ $group_date ] : array();
	$is_group_available    = ! empty( $group_bookings_data[ 'is_available' ] ) ? $group_bookings_data[ 'is_available' ] : 0;
	$group_availability    = ! empty( $group_bookings_data[ 'availability' ] ) ? $group_bookings_data[ 'availability' ] : 0;
	$current_user_bookings = ! empty( $group_bookings_data[ 'current_user_bookings' ] ) ? $group_bookings_data[ 'current_user_bookings' ] : 0;
	$distinct_users        = ! empty( $group_bookings_data[ 'distinct_users' ] ) ? $group_bookings_data[ 'distinct_users' ] : 0;
	
	$is_available = $group_data && $group_events && $is_group_available && $group_availability > 0;
	
	if( $is_available ) {
		$category_id   = $group_data[ 'category_id' ];
		$category_data = ! empty( $booking_system_data[ 'group_categories_data' ][ $category_id ][ 'settings' ] ) ? $booking_system_data[ 'group_categories_data' ][ $category_id ][ 'settings' ] : array();
		$started_groups_bookable = bookacti_get_setting_value( 'bookacti_general_settings', 'started_groups_bookable' ) ? 1 : 0;
		if( isset( $category_data[ 'started_groups_bookable' ] ) && in_array( $category_data[ 'started_groups_bookable' ], array( 0, 1, '0', '1', true, false ), true ) ) {
			$started_groups_bookable = intval( $category_data[ 'started_groups_bookable' ] );
		}
		...
		$group_start  = DateTime::createFromFormat( 'Y-m-d H:i:s', $first_event[ 'start' ], $timezone );
		$group_end    = DateTime::createFromFormat( 'Y-m-d H:i:s', $last_event[ 'end' ], $timezone );
		$past_events_bookable = $booking_system_data[ 'past_events' ] && $booking_system_data[ 'past_events_bookable' ];
		
		if( ! $past_events_bookable && $group_start < $now
		&& ! ( $started_groups_bookable && $group_end > $now ) ) {
			$is_available = false;
		}
		...
		if( $is_available ) {
			...
			// Check if the group of events is in the availability period
			if( ( $availability_period[ 'start' ] && $group_start < $availability_period[ 'start' ] && $group_start > $now )
			||  ( $availability_period[ 'end' ] && $group_start > $availability_period[ 'end' ] ) ) { 
				$is_available = false;
			}
		}
		...
		if( $is_available && $booking_system_data[ 'days_off' ] ) {
			$started_days_off_bookable = bookacti_get_setting_value( 'bookacti_general_settings', 'started_days_off_bookable' );
			foreach( $group_events as $group_event ) {
				$group_event_start_dt = new DateTime( $group_event[ 'start' ], $timezone );
				$group_event_end_dt   = new DateTime( $group_event[ 'end' ], $timezone );
				foreach( $booking_system_data[ 'days_off' ] as $off_period ) {
					$off_from = new DateTime( $off_period[ 'from' ] . ' 00:00:00', $timezone );
					$off_to   = new DateTime( $off_period[ 'to' ] . ' 23:59:59', $timezone );
					if( ( $group_event_start_dt >= $off_from && $group_event_start_dt <= $off_to )
					||  ( ! $started_days_off_bookable && $group_event_start_dt < $off_from && $group_event_end_dt > $off_from ) ) {
						$is_available = false;
						break;
					}
				}
		...
		if( $is_available ) {
			...
			// Check the min and max quantity allowed AND
			// Check the max number of different users allowed
			$min_qty_ok = false; $max_qty_ok = true; $max_users_ok = true;
			...
			if( $max_quantity || $max_users ) {
				$qty_booked = intval( $current_user_bookings );
				if( $max_users && $qty_booked === 0 && intval( $distinct_users ) >= $max_users ) {
					$max_users_ok = false;
				}
				if( $max_quantity && $qty_booked >= $max_quantity ) {
					$max_qty_ok = false;
...
function bookacti_get_groups_of_events_array_from_db_groups_of_events( $groups, $raw_args = array() ) {
	...
	foreach( $groups as $group ) {
		$group->repeat_exceptions = ! empty( $group->repeat_exceptions ) ? maybe_unserialize( $group->repeat_exceptions ) : array();
		if( ! is_array( $group->repeat_exceptions ) ) { $group->repeat_exceptions = array(); }
		...
		$groups_array[ 'data' ][ $group_id ][ 'settings' ][ 'started_groups_bookable' ] = isset( $group->started_groups_bookable ) && in_array( $group->started_groups_bookable, array( 0, 1, '0', '1', true, false ), true ) ? intval( $group->started_groups_bookable ) : $started_groups_bookable;
	}
 ...
 * @return array
 */
function bookacti_get_bounding_events_from_groups_of_events_heuristic( $groups, $raw_args = array() ) {
	if( empty( $groups[ 'data' ] ) ) { return array(); }
	...
	foreach( $groups[ 'data' ] as $group_id => $group ) {
		// For the repeated groups, get the events of the first and the last theorical groups
		if( $group[ 'repeat_freq' ] && $group[ 'repeat_freq' ] !== 'none' ) {
			...
			$first_event_start_dt = new DateTime( $first_event[ 'start' ], $timezone );
			foreach( $first_event_occurrences as $first_event_occurrence ) {
				$first_event_occurrence_dt = new DateTime( $first_event_occurrence[ 'start' ], $timezone );
				...
				foreach( $group[ 'events' ] as $group_event ) {
					$group_event_start_dt = new DateTime( $group_event[ 'start' ], $timezone );
					$group_event_end_dt   = new DateTime( $group_event[ 'end' ], $timezone );
					$delta    = bookacti_php_date_interval_in_seconds( $first_event_start_dt->diff( $group_event_start_dt, true ) );
					$duration = bookacti_php_date_interval_in_seconds( $group_event_start_dt->diff( $group_event_end_dt, true ) );
					...
					$desired_event = array_merge( $group_event, array( 'start' => $desired_event_start_dt->format( 'Y-m-d H:i:s' ), 'end' => $desired_event_end_dt->format( 'Y-m-d H:i:s' ) ) );
					$grouped_events[] = $desired_event;
					
					if( $args[ 'groups_first_event_only' ] ) { break; }
				}
			}
			
		// For non repeated groups, get their events
		} else {
			$group_events = $group[ 'events' ];
			if( $args[ 'groups_first_event_only' ] ) {
				$group_events = array();
				if( ! empty( $group[ 'events' ] ) ) {
					$group_events[] = reset( $group[ 'events' ] );
				}
			}
			$grouped_events = array_merge( $grouped_events, $group_events );
		}
	}
	
	// Get the bounding events
	$bounding_dt = array();
	$bounding_events = array();
	foreach( $grouped_events as $i => $grouped_event ) {
		$grouped_event_start_dt = new DateTime( $grouped_event[ 'start' ], $timezone );
	...
	// Keep only one event if it is both the first and the last, and remove the array keys
	if( $bounding_events ) {
		$bounding_events = $bounding_events[ 'first' ] == $bounding_events[ 'last' ] ? array( $bounding_events[ 'first' ] ) : array( $bounding_events[ 'first' ], $bounding_events[ 'last' ] );
	}
	
	return apply_filters( 'bookacti_bounding_events_from_groups_of_events_heuristic', $bounding_events, $groups, $args );
}
...
function bookacti_get_occurrences_of_repeated_groups_of_events( $groups, $raw_args ) {
	...
	foreach( $groups as $group_id => $group ) {
		...
		$first_event_dt = DateTime::createFromFormat( 'Y-m-d H:i:s', $first_event[ 'start' ], $timezone );
		$last_event_dt  = DateTime::createFromFormat( 'Y-m-d H:i:s', $last_event[ 'end' ], $timezone );
		$group_duration = bookacti_php_date_interval_in_seconds( $first_event_dt->diff( $last_event_dt, true ) );
		$repeat_to_dt->add( $group_duration );
		$repeat_to_dt->setTime( 23, 59, 59 );
		
		if( ! $repeated_interval_start_dt || ( $repeated_interval_start_dt && $repeat_from_dt < $repeated_interval_start_dt ) ) { $repeated_interval_start_dt = $repeat_from_dt; }
		if( ! $repeated_interval_end_dt || ( $repeated_interval_end_dt && $repeat_to_dt > $repeated_interval_end_dt ) ) { $repeated_interval_end_dt = $repeat_to_dt; }
	}
	...
	$repeated_args[ 'interval' ] = array( 
		'start' => $repeated_interval_start_dt ? $repeated_interval_start_dt->format( 'Y-m-d H:i:s' ) : '', 
		'end'   => $repeated_interval_end_dt ? $repeated_interval_end_dt->format( 'Y-m-d H:i:s' ) : ''
	);
	...
	foreach( $groups as $group_id => $group ) {
		...
		// Get the occurrences of the first event of the group (it will determine the other group occurrences)
		$dummy_args = array( 
			'interval'        => $args[ 'interval' ],
...
function bookacti_get_number_of_bookings_per_group_of_events( $groups, $raw_bookings_args = array() ) {
	...
	if( $bookings_args[ 'users' ] ) { $bookings_args[ 'in__user_id' ] = $bookings_args[ 'users' ]; }
	...
	foreach( $groups[ 'groups' ] as $group_id => $groups_per_dates ) {
		foreach( $groups_per_dates as $group_date => $group_events ) {
			foreach( $group_events as $group_event ) {
	...
	$bookings_filters = bookacti_format_booking_filters( 
		array_merge( $bookings_args, array(
