		...
		public function get_booking_list_items() {
			...
			foreach( $bookings as $booking ) {
				...
				$booking_item = apply_filters( 'bookacti_booking_list_booking_columns', array( 
					...
					'quantity'          => $quantity,
					'price_details'     => array(),
					'event_title'       => $title ? apply_filters( 'bookacti_translate_text', $title ) : $title,
					'start_date'        => bookacti_format_datetime( $start, $datetime_format ),
					'end_date'          => bookacti_format_datetime( $end, $datetime_format ),
					'template_title'    => $booking->template_title ? apply_filters( 'bookacti_translate_text', $booking->template_title ) : $booking->template_title,
			...
			foreach( $booking_list_items as $booking_id => $booking_list_item ) {
				...
				if( $booking_list_item[ 'booking_type' ] === 'group' ) {
					$booking_list_items[ $booking_id ][ 'actions' ] = ! empty( $booking_list_item[ 'actions' ] ) ? bookacti_get_booking_group_actions_html( $booking_groups[ $booking_list_item[ 'raw_group_id' ] ], $bookings_per_group[ $booking_list_item[ 'raw_id' ] ], 'admin', $booking_list_item[ 'actions' ] ) : '';
