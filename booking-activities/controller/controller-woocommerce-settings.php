...
function bookacti_wc_declare_compatibility() {
	...
	foreach( $plugins as $prefix => $plugin ) {
		...
		\Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility( 'cart_checkout_blocks', $plugin[ 'plugin_name' ] . '/' . $plugin[ 'plugin_name' ] . '.php', true );
