...
function bookacti_controller_generate_export_bookings_url() {
	...
	// Add iCal specific args
		...
		$add_url_atts[ 'vevent_summary' ]      = urlencode( bookacti_utf8_encode( trim( $export_settings[ 'vevent_summary' ] ) ) );
		$add_url_atts[ 'vevent_description' ]  = urlencode( bookacti_utf8_encode( str_replace( array( PHP_EOL, '\n' ), '%0A', trim( $export_settings[ 'vevent_description' ] ) ) ) );
